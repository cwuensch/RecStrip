...
static const word TTXT_COLORSYMBOLS[2][8] = {
  { 0x25CF, 0x25D0, 0x25D1, 0x25D2, 0x25D3, 0x25D4, 0x25D5, 0x25CB },  // Foreground: ‚óè Black, ‚óê Red, ‚óë Green, ‚óí Yellow, ‚óì Blue, ‚óî Magenta, ‚óï Cyan, ‚óã White*
  { 0x25A3, 0x25A4, 0x25A5, 0x25A6, 0x25A7, 0x25A8, 0x25A9, 0x25A1 }   // Background: ‚ñ£ Black*, ‚ñ§ Red, ‚ñ• Green, ‚ñ¶ Yellow, ‚ñß Blue, ‚ñ® Magenta, ‚ñ© Cyan, ‚ñ° White
};
...
//  if (!page_is_duplicate && (page->show_timestamp <= page->last_show_timestamp + 100))
  {
    // Erst hier das Output-File ˆffnen
    if (!fTtxOut[out_nr])
    {
  ...
  if (page_map[page_nr-100].subpages[p] >= 0)
  {
    ...
    for (s = p; s < NRSUBPAGES; s++)
    {
      ...
      for (i = 0; i < 25; i++)
      {
        ...
        for (j = 0; j < 40; j++)
        {
          if (page->text[i][j] == ref->text[i][j])  nr_content_same++;  // vˆllige ‹bereinstimmung
          else
          {
...
//packet->data[i] = packet->data[i] & 0x7f;
        ...
        if(line_empty)  // CW: line_empty eher unnˆtig - eingef¸hrt, um Wiederholung von Untertiteln vermeiden zu kˆnnen
//          for (i = 0; i < 40; i++)
...
/*if (p == 83 && s == 1)
{
  FILE *fDbg = fopen("C:/Users/Christian W¸nsch/Downloads/Lagunen_p183.txt", "wb");
  printf("page 183\n");
  for (i = 0; i < 24; i++)
  {
    for (j = 0; j < 40; j++)
  ...
  fclose(fDbg);
      ...
      for (i = 0; i < 24; i++)
      {
        ...
        // Falls doppelte Hˆhe, die n‰chste Zeile ignorieren
        if (double_height)
        {
          double_height = FALSE;
//          fwrite("\r\n", 1, 1, f);
...
//              last_c = ' ';
        ...
        // Falls doppelte Hˆhe, die n‰chste Zeile ignorieren (auﬂer Hintergrund)
        if (double_height)
        {
          for (j = 0; j < col_stop; j++)
