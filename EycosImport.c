...
//          case STREAM_VIDEO_VC1SM:
          ...
          case 0x0a:
          {
            ...
            Offset           += (sizeof(tElemStream) + Elem->ESInfoLen2);
...
if (strlen(RecInf->ExtEventInfo.Text) != RecInf->ExtEventInfo.TextLength)
  printf("ASSERT: ExtEventTextLength (%d) != length of ExtEventText (%d)!\n", RecInf->ExtEventInfo.TextLength, strlen(RecInf->ExtEventInfo.Text));
      ...
      RecInf->EventInfo.StartTime       = Unix2TFTime(EvtStartUnix, NULL);  // DATE(UnixToMJD(EvtStartUnix), EycosEvent.EvtStartHour, EycosEvent.EvtStartMin);
      RecInf->EventInfo.EndTime         = Unix2TFTime(EvtEndUnix, NULL);    // DATE(UnixToMJD(EvtEndUnix), EycosEvent.EvtEndHour, EycosEvent.EvtEndMin);
#ifdef _DEBUG
